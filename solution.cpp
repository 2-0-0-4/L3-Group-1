    ...
    while (true)
    {
        ...
        SensorReading reading;
        {
            ...
            if (!readingQueue.empty())
            {
                ...
                std::vector<SensorReading> top_min = Stream.top_k_min(1);
                std::vector<SensorReading> top_max = Stream.top_k_max(1);

                // ALERT if any threshold crossed:

                if (top_min[0].temperature < 15)
                {
                    outfile << "[ALERT] Time: " << top_min[0].timestamp << " | Sensor: " << top_min[0].sensorID << " | Type: Lower Threshold Crossed | Temp: " << top_min[0].temperature << " C\n";
                }

                if (top_max[0].temperature > 48)
                {
                    outfile << "[ALERT] Time: " << top_min[0].timestamp << " | Sensor: " << top_min[0].sensorID << " | Type: Upper Threshold Crossed | Temp: " << top_min[0].temperature << " C\n";
                }
